...
public:
    ...
    void rooms(bool includeChildren, const std::function<void(json)>& ok, const LokatoDatabase::Error& error) {
        ...
        database.query(
            sql,
            [this, includeChildren, result, ok, error](MYSQL_ROW row) {
                if (row != nullptr) {
    ...
    void roomOccupancy(int roomId, const std::function<void(json)>& ok, const LokatoDatabase::Error& error) {
        ...
        database.query(
            "SELECT id,name,area,capacity,tolerance FROM rooms WHERE id=" + std::to_string(roomId) + " LIMIT 1",
            [this, roomId, payload, ok, error](MYSQL_ROW row) {
                if (row != nullptr) {
                    (*payload)["room"] = {{"id", toInt(row[0])},
                                          {"name", toString(row[1])},
                                          {"area", row[2] == nullptr ? json(nullptr) : json(toString(row[2]))},
                                          {"capacity", toInt(row[3])},
                                          {"tolerance", toInt(row[4])}};
                    return;
                }
                ...
                database.query(
                    "SELECT c.id,c.name,c.photo_url,c.tracker_uid,c.is_active,cl.updated_at "
                    "FROM child_locations cl JOIN children c ON c.id=cl.child_id "
                    "WHERE cl.room_id=" +
                        std::to_string(roomId) + " ORDER BY c.name",
                    [payload, ok](MYSQL_ROW childRow) {
                        if (childRow != nullptr) {
                            (*payload)["children"].push_back({{"child_id", toInt(childRow[0])},
                                                               {"id", toInt(childRow[0])},
                                                               {"name", toString(childRow[1])},
                                                               {"photo_url", childRow[2] == nullptr ? json(nullptr) : json(toString(childRow[2]))},
                                                               {"tracker_uid", childRow[3] == nullptr ? json(nullptr) : json(toString(childRow[3]))},
                                                               {"is_active", toBool(childRow[4])},
                                                               {"updated_at", childRow[5] == nullptr ? json(nullptr) : json(toString(childRow[5]))}});
                            return;
                        }
...
private:
    void attachChildren(json& rooms, const std::function<void(json)>& ok, const LokatoDatabase::Error& error) {
        ...
        database.query(
            "SELECT cl.room_id,c.id,c.name,c.photo_url,c.tracker_uid,c.is_active,cl.updated_at "
            "FROM child_locations cl JOIN children c ON c.id=cl.child_id ORDER BY c.name",
            [roomsPtr, ok](MYSQL_ROW row) {
                if (row != nullptr) {
                    const int roomId = toInt(row[0]);
                    for (auto& room : *roomsPtr) {
                        if (room["id"].get<int>() == roomId) {
                            room["children"].push_back({{"child_id", toInt(row[1])},
                                                         {"id", toInt(row[1])},
                                                         {"name", toString(row[2])},
                                                         {"photo_url", row[3] == nullptr ? json(nullptr) : json(toString(row[3]))},
                                                         {"tracker_uid", row[4] == nullptr ? json(nullptr) : json(toString(row[4]))},
                                                         {"is_active", toBool(row[5])},
                                                         {"updated_at", row[6] == nullptr ? json(nullptr) : json(toString(row[6]))}});
                            break;
                        }
                    }
                    return;
...
public:
    ...
    void ingest(const std::string& deviceKey,
                const std::string& trackerUid,
                const std::string& eventTimeIso,
                const std::string& source,
                const std::function<void(json)>& ok,
                const std::function<void(int, json)>& fail) {
        ...
        database.query(
            "SELECT id,room_id FROM devices WHERE device_key=" + sqlQuote(deviceKey) + " LIMIT 1",
            [this, ctx, ok, fail](MYSQL_ROW row) {
                if (row != nullptr) {
...
private:
    ...
    void lookupChild(const std::shared_ptr<Context>& ctx, const std::function<void(json)>& ok, const std::function<void(int, json)>& fail) {
        database.query(
            "SELECT id FROM children WHERE tracker_uid=" + sqlQuote(ctx->trackerUid) + " LIMIT 1",
            [this, ctx, ok, fail](MYSQL_ROW row) {
                if (row != nullptr) {
    ...
    void lookupCurrentLocation(const std::shared_ptr<Context>& ctx, const std::function<void(json)>& ok, const std::function<void(int, json)>& fail) {
        database.query(
            "SELECT room_id FROM child_locations WHERE child_id=" + std::to_string(ctx->childId) + " LIMIT 1",
            [this, ctx, ok, fail](MYSQL_ROW row) {
                if (row != nullptr) {
    ...
    void persist(const std::shared_ptr<Context>& ctx, const std::function<void(json)>& ok, const std::function<void(int, json)>& fail) {
        const std::string insertMovement =
            "INSERT INTO movement_log(child_id,from_room_id,to_room_id,device_id,source,occurred_at) VALUES (" +
            std::to_string(ctx->childId) + "," + sqlNullableInt(ctx->fromRoomId) + "," + std::to_string(ctx->toRoomId) + "," +
            std::to_string(ctx->deviceId) + "," + sqlQuote(ctx->source) + "," + sqlQuote(ctx->eventTime) + ")";

        database.raw()
            .exec("START TRANSACTION", [] {}, [fail](const std::string& msg, unsigned int no) {
                fail(500, {{"error", "db_error"}, {"message", msg}, {"code", no}});
            })
            .exec(insertMovement, [] {}, [fail](const std::string& msg, unsigned int no) {
                fail(500, {{"error", "db_error"}, {"message", msg}, {"code", no}});
            })
            .query(
    ...
    void updateState(const std::shared_ptr<Context>& ctx, const std::function<void(json)>& ok, const std::function<void(int, json)>& fail) {
        const std::string upsertLocation =
            "INSERT INTO child_locations(child_id,room_id,updated_at) VALUES (" + std::to_string(ctx->childId) + "," +
            std::to_string(ctx->toRoomId) + "," + sqlQuote(ctx->eventTime) + ") "
            "ON DUPLICATE KEY UPDATE "
            "room_id=IF(VALUES(updated_at) >= updated_at, VALUES(room_id), room_id), "
            "updated_at=GREATEST(updated_at, VALUES(updated_at))";

        database.raw()
            .exec(upsertLocation, [] {}, [fail](const std::string& msg, unsigned int no) {
                fail(500, {{"error", "db_error"}, {"message", msg}, {"code", no}});
            })
            .exec("UPDATE children SET is_active=1 WHERE id=" + std::to_string(ctx->childId), [] {}, [fail](const std::string& msg, unsigned int no) {
                fail(500, {{"error", "db_error"}, {"message", msg}, {"code", no}});
            })
            .exec("UPDATE devices SET last_seen=NOW() WHERE id=" + std::to_string(ctx->deviceId), [] {}, [fail](const std::string& msg, unsigned int no) {
                fail(500, {{"error", "db_error"}, {"message", msg}, {"code", no}});
            })
            .exec(
                "COMMIT",
                [this, ctx, ok]() {
                    const json movement = {{"id", ctx->movementId},
                                           {"child_id", ctx->childId},
                                           {"from_room_id", ctx->fromRoomId == 0 ? json(nullptr) : json(ctx->fromRoomId)},
                                           {"to_room_id", ctx->toRoomId},
                                           {"device_id", ctx->deviceId},
                                           {"source", ctx->source},
                                           {"occurred_at", ctx->eventTime}};
                    sse.broadcast("child.moved", movement, "movement:" + std::to_string(ctx->movementId) + ";alert:0");
                    occupancy.roomOccupancy(ctx->toRoomId,
                                            [this, ctx](json payload) {
                                                payload["room_id"] = ctx->toRoomId;
                                                if (payload.contains("room") && payload["room"].contains("name")) {
                                                    payload["room_name"] = payload["room"]["name"];
                                                }
                                                sse.broadcast("room.occupancy.updated", payload,
                                                              "movement:" + std::to_string(ctx->movementId) + ";alert:0");
                                            },
...
public:
    explicit LokatoMqtt(const std::string& connectionName)
        : iot::mqtt::client::Mqtt(connectionName,
                                  globalConfig != nullptr ? globalConfig->mqttClientId : "lokato-snode-subscriber",
                                  30,
                                  "") {
    }
...
private:
    ...
    void onPublish(const iot::mqtt::packets::Publish& publish) override {
        ...
        try {
            ...
            globalScanIngestService->ingest(payload.value("device_key", ""),
                                            payload.value("tracker_uid", ""),
                                            payload.value("event_time", ""),
                                            "mqtt_scanner",
...
void registerRoutes(const WebApp& app, const AppConfig& config, OccupancyService& occupancy, ScanIngestService& scans, LokatoDatabase& database, SseHub& sse) {
    ...
    app.get("/api/v1/rooms", [&occupancy] APPLICATION(req, res) {
        occupancy.rooms(req->queries["include_children"] == "true" || req->queries["include_children"] == "1",
    ...
    app.get("/api/v1/rooms/:room/occupancy", [&occupancy] APPLICATION(req, res) {
        occupancy.roomOccupancy(std::stoi(req->params["room"]),
                                [res](json payload) {
    ...
    app.post("/api/v1/scan", [&scans] APPLICATION(req, res) {
        try {
            ...
            scans.ingest(payload.value("device_key", ""),
                         payload.value("tracker_uid", ""),
                         payload.value("event_time", ""),
                         "device",
                         [res](json result) { res->json(result); },
                         [res](int status, json error) { res->status(status).json(error); });
        } catch (const std::exception& e) {
    ...
    app.get("/api/v1/movement-log", [&database] APPLICATION(req, res) {
        ...
        database.query(
            "SELECT ml.id,ml.child_id,c.name,fr.id,fr.name,tr.id,tr.name,d.id,d.name,ml.source,ml.occurred_at "
            "FROM movement_log ml "
            "JOIN children c ON c.id=ml.child_id "
            "LEFT JOIN rooms fr ON fr.id=ml.from_room_id "
            "LEFT JOIN rooms tr ON tr.id=ml.to_room_id "
            "LEFT JOIN devices d ON d.id=ml.device_id "
            "ORDER BY ml.occurred_at DESC, ml.id DESC LIMIT " +
                std::to_string(perPage),
            [data, res](MYSQL_ROW row) {
                if (row != nullptr) {
                    data->push_back({{"id", toInt(row[0])},
                                     {"child", {{"id", toInt(row[1])}, {"name", toString(row[2])}}},
                                     {"from_room", row[3] == nullptr ? json(nullptr) : json({{"id", toInt(row[3])}, {"name", toString(row[4])}})},
                                     {"to_room", row[5] == nullptr ? json(nullptr) : json({{"id", toInt(row[5])}, {"name", toString(row[6])}})},
                                     {"device", row[7] == nullptr ? json(nullptr) : json({{"id", toInt(row[7])}, {"name", toString(row[8])}})},
                                     {"source", toString(row[9])},
                                     {"occurred_at", toString(row[10])}});
                    return;
                }
    ...
    app.get("/api/v1/admin/summary", [&database] APPLICATION(req, res) {
        ...
        database.raw()
            .query("SELECT COUNT(*) FROM children", [result](MYSQL_ROW row) {
                if (row != nullptr) (*result)["children_count"] = toInt(row[0]);
            }, [](const std::string&, unsigned int) {})
            .query("SELECT COUNT(*) FROM rooms", [result](MYSQL_ROW row) {
                if (row != nullptr) (*result)["rooms_count"] = toInt(row[0]);
            }, [](const std::string&, unsigned int) {})
            .query("SELECT COUNT(*) FROM devices", [result, res](MYSQL_ROW row) {
                if (row != nullptr) (*result)["devices_count"] = toInt(row[0]);
                if (row == nullptr) res->json(*result);
            }, [res](const std::string& msg, unsigned int no) { jsonError(res, 500, "db_error", msg + " [" + std::to_string(no) + "]"); });
    });
    ...
    app.post("/api/v1/children/:child/checkout", [&database, &sse, &occupancy] APPLICATION(req, res) {
        ...
        database.raw()
            .query("SELECT room_id FROM child_locations WHERE child_id=" + std::to_string(childId) + " LIMIT 1", [fromRoom](MYSQL_ROW row) {
                if (row != nullptr) *fromRoom = toInt(row[0]);
            }, [](const std::string&, unsigned int) {})
            ...
            .exec("INSERT INTO movement_log(child_id,from_room_id,to_room_id,device_id,source,occurred_at) VALUES (" + std::to_string(childId) + "," +
                      sqlNullableInt(*fromRoom) + ",NULL,NULL,'manual',NOW())",
                  [] {}, [](const std::string&, unsigned int) {})
            .query("SELECT LAST_INSERT_ID()", [fromRoom, childId, res, &sse, &occupancy](MYSQL_ROW row) {
                if (row != nullptr) {
                    ...
                    if (*fromRoom > 0) {
                        occupancy.roomOccupancy(*fromRoom, [&sse, movementId, fromRoom](json payload) {
                            payload["room_id"] = *fromRoom;
                            sse.broadcast("room.occupancy.updated", payload, "movement:" + std::to_string(movementId) + ";alert:0");
                        }, [](const std::string&, unsigned int) {});
                    }
    ...
    app.get("/api/stream/room/:room", [&occupancy] APPLICATION(req, res) {
        ...
        res->set("Transfer-Encoding", "chunked");
        res->sendHeader();
        occupancy.roomOccupancy(roomId,
                                [res, roomId](json payload) {
...
int main(int argc, char* argv[]) {
    ...
    app.listen(config.httpPort, [](const WebApp::SocketAddress& socketAddress, const core::socket::State& state) {
        switch (state) {
            case core::socket::State::OK:
                VLOG(1) << "lokato-snode HTTP listening on " << socketAddress.toString();
                break;
            case core::socket::State::DISABLED:
                VLOG(1) << "lokato-snode HTTP disabled";
                break;
            case core::socket::State::ERROR:
                LOG(ERROR) << "lokato-snode HTTP " << socketAddress.toString() << ": " << state.what();
                break;
            case core::socket::State::FATAL:
                LOG(FATAL) << "lokato-snode HTTP " << socketAddress.toString() << ": " << state.what();
                break;
        }
    });
    ...
    mqttClient.connect([](const MqttClient::SocketAddress& socketAddress, const core::socket::State& state) {
        switch (state) {
            case core::socket::State::OK:
                VLOG(1) << "lokato MQTT connected to " << socketAddress.toString();
                break;
            case core::socket::State::DISABLED:
                VLOG(1) << "lokato MQTT disabled";
                break;
            case core::socket::State::ERROR:
                LOG(ERROR) << "lokato MQTT " << socketAddress.toString() << ": " << state.what();
                break;
            case core::socket::State::FATAL:
                LOG(FATAL) << "lokato MQTT " << socketAddress.toString() << ": " << state.what();
                break;
        }
    });
